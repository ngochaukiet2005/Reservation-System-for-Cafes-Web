import { Controller, Get, Post, Body, Patch, Delete, Param, UseGuards, Query, Request } from '@nestjs/common';
...
export class ReservationsController {
  ...
  async findAll(@Query('status') status?: string, @Query('date') date?: string) {
    const reservations = await this.reservationsService.findAll({ status, date });
  ...
  @Patch(':id/cancel')
  async cancel(@Param('id') id: string, @Body() dto: any) {
    const reservation = await this.reservationsService.cancel(id, dto.reason);
