      ...
      <div v-if="showDetailModal && selectedDetailRes" class="modal-overlay" @click.self="showDetailModal = false">
        <div class="modal-content info-modal">
          ...
          <div class="detail-row">
            <strong>👤 Khách hàng:</strong> {{ selectedDetailRes.guestName }}
          </div>
          <div class="detail-row">
            <strong>📞 SĐT:</strong> {{ selectedDetailRes.phone }}
          </div>
          <div class="detail-row">
            <strong>🕒 Thời gian:</strong> {{ formatDate(selectedDetailRes.time) }} - {{ formatTime(selectedDetailRes.time) }}
          </div>
          <div class="detail-row">
             <strong>📍 Bàn:</strong> {{ selectedDetailRes.tableName }} ({{ selectedDetailRes.people }} người)
          </div>
          
          <div class="detail-row reason-box" v-if="selectedDetailRes.cancellationReason">
             <strong>Lý do hủy của bạn:</strong> <br>
             "{{ selectedDetailRes.cancellationReason }}"
          </div>

           <div class="detail-row reason-box admin" v-if="selectedDetailRes.adminResponse">
             <strong>💬 Phản hồi từ Staff:</strong> <br>
             "{{ selectedDetailRes.adminResponse }}"
          </div>

          <div class="modal-actions">
            <button class="btn-primary" @click="showDetailModal = false">Đóng</button>
          </div>
        </div>
      </div>
    </transition>
...
const viewDetails = (res: any) => {
  selectedDetailRes.value = res;
...
const onCancelClick = async (res: any) => {
  ...
  if (now > reservationTime + holdTimeMs) {
    Swal.fire({
      icon: 'error',
      title: 'Không thể hủy',
      text: 'Đã quá thời gian giữ bàn. Vui lòng liên hệ hotline.',
    });
    return;
  }
  ...
  if (res.status === 'PENDING') {
    const result = await Swal.fire({
      title: 'Hủy đơn chờ?',
      text: "Bạn có chắc chắn muốn hủy đơn đang chờ duyệt này không?",
      icon: 'warning',
...
const confirmCancelRequest = async () => {
  if (!cancelReason.value.trim()) {
    Swal.fire('Thiếu thông tin', 'Vui lòng nhập lý do hủy!', 'warning');
    return;
  }
  ...
  if (selectedCancelRes.value) {
    ...
    Swal.fire({
      ...
      text: 'Yêu cầu hủy đang chờ nhân viên xác nhận.',
      timer: 2000,
...
/* [MỚI] CSS cho phần Tên bàn + Icon i */
.table-name-wrapper {
  display: flex;
  align-items: center;
...
.btn-icon.info-inline { 
  font-size: 1rem; 
  opacity: 0.6; /* Làm mờ nhẹ cho đỡ rối mắt */
}
