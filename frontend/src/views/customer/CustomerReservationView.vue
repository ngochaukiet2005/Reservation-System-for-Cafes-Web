  ...
  <div class="reservation-page">
      ...
      <div class="filters">
              ...
              <div v-if="showMinuteDropdown" class="time-picker-dropdown">
                ...
                <div class="time-section">
                  ...
                  <div class="time-scroll">
                    ...
                    <div
                      v-if="availableMinutes.length === 0"
                      class="time-empty"
                    >
                      Hết giờ
                    </div>
        ...
        <div class="filter-item action-col">
          ...
          <button
            ...
            @click="suggestTableAfterSearch"
            :disabled="reservationStore.tables.length === 0 || isSearching"
          >
            ⚡ CHỌN NHANH
          </button>
        </div>
    ...
    <transition name="slide-up">
      <div v-if="selectedTable" class="footer-action-fixed">
        ...
        <button class="btn-continue" @click="showForm = true">
          Điền Thông Tin ➝
        </button>
      </div>
    </transition>
...
const suggestTableAfterSearch = () => {
  ...
  if (filter.people === 0) {
    ...
    bestTable = reservationStore.tables.find(
      (t) => t.status === "AVAILABLE" && t.capacity >= filter.people,
    );
  }
...
const loadTables = async (manualTrigger = false) => {
  ...
  try {
    ...
    const allTables = await reservationStore.fetchTablesByTime(
      filter.date,
    ...
    if (filter.people === 0) {
      ...
      if (filter.people === 2) {
        ...
        filteredTables = filteredTables.filter(
          (table: any) => table.capacity >= 1 && table.capacity <= 2,
        );
      } else if (filter.people === 4) {
        // 3-4 người: chỉ lấy bàn capacity 3-4
        filteredTables = filteredTables.filter(
          (table: any) => table.capacity >= 3 && table.capacity <= 4,
        );
      } else if (filter.people === 6) {
        // 5+ người: chỉ lấy bàn capacity >= 5
        filteredTables = filteredTables.filter(
          (table: any) => table.capacity >= 5,
        );
      }
    ...
    if (reservationStore.tables.length === 0) {
      alert(
        "Không tìm thấy bàn trong thời gian này. Vui lòng chọn thời gian khác.",
      );
    }
    ...
    if (selectedTable.value && !manualTrigger) {
      const stillValid = reservationStore.tables.find(
        (t: any) =>
          t.id === selectedTable.value?.id && t.status === "AVAILABLE",
      );
      if (!stillValid) {
        selectedTable.value = null;
      }
  ...
  if (!customerSocket) {
    ...
    customerSocket.on("table.updated", (updatedTable: any) => {
      ...
      if (updatedTable && updatedTable.id) {
        const index = reservationStore.tables.findIndex(
          (t: any) => t.id === updatedTable.id,
        );
        if (index !== -1) {
          ...
          console.log(
            `[CustomerReservation] Updated table ${updatedTable.id} to status ${updatedTable.status?.name}`,
          );
        }
      }
    });
